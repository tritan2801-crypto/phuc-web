...
import { MOCK_PRODUCTS, Product, MOCK_COMBOS } from '@core/constants/mock-data'

export default function ProductDetailPage() {
  const params = useParams()
  const { addToCart, setIsCartOpen, isB2b, activeBranch } = useApp()
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8 font-sans">
      ...
      <div className="flex items-center gap-1.5 text-xs text-slate-400 font-semibold mb-6">
        <Link href="/" className="hover:text-primary-500 transition-colors">Khang Phúc</Link>
        <ChevronRight size={12} />
        <Link href="/products" className="hover:text-primary-500 transition-colors">Sản phẩm</Link>
        <ChevronRight size={12} />
