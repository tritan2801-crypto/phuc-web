...
export default function AdminProducts() {
  ...
  return (
    <div className="space-y-6 font-sans text-slate-200 selection:bg-teal-500 selection:text-slate-950">
        ...
        <div className="flex flex-1 flex-col sm:flex-row gap-3">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl py-2.5 pl-10 pr-4 text-xs text-white placeholder-slate-600 focus:border-teal-500 focus:outline-none transition-all"
            />
          </div>
          ...
          <select
            ...
            className="bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-xs text-slate-350 focus:border-teal-500 focus:outline-none transition-all cursor-pointer font-semibold"
          >
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        {loading ? (
          ...
          <div className="py-16 text-center text-slate-500 space-y-2">
            <Package className="mx-auto text-slate-650" size={48} />
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-850/50 text-slate-400 font-extrabold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="p-4 text-right">Đơn giá bán lẻ</th>
                  <th className="p-4 text-right">Đơn giá đại lý</th>
                ...
                {filteredProducts.map((prod) => (
                  <tr key={prod.id} className="hover:bg-slate-850/20 transition-colors">
                    <td className="p-4 pl-6 flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-slate-950 border border-slate-800 flex items-center justify-center text-[10px] text-teal-400 font-bold overflow-hidden uppercase">
                    ...
                    <td className="p-4 font-mono text-[11px] text-slate-400">{prod.sku}</td>
                    ...
                    <td className="p-4 text-right text-teal-400 font-bold">{prod.price.toLocaleString('vi-VN')} ₫</td>
                    <td className="p-4 text-right text-blue-400 font-bold">{prod.agentPrice.toLocaleString('vi-VN')} ₫</td>
                    <td className="p-4 pr-6 text-center">
                      <div className="flex justify-center gap-2">
                        <button
                          onClick={() => handleOpenEditModal(prod)}
                          className="p-2 bg-slate-800 hover:bg-slate-750 text-teal-400 hover:text-teal-300 border border-slate-700 rounded-lg cursor-pointer transition-colors"
                        ...
                        <button
                          ...
                          className="p-2 bg-slate-800 hover:bg-red-950/20 text-red-400 hover:text-red-300 border border-slate-700 rounded-lg cursor-pointer transition-colors"
                          title="Xóa sản phẩm"
                        >
                          <Trash2 size={13} />
                        </button>
