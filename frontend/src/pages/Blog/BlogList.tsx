    ...
    <div className="max-w-5xl mx-auto px-4 py-8 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {MOCK_BLOGS.map((blog) => (
          <div
            key={blog.id}
            className="bg-white rounded-2xl border border-slate-150 shadow-sm overflow-hidden hover:shadow-md transition-all duration-300 flex flex-col justify-between group"
          >
            <Link to={`/blog/${blog.slug}`} className="block h-48 bg-slate-100 relative overflow-hidden">
            ...
            <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
              ...
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-400 font-semibold">
                ...
                <Link to={`/blog/${blog.slug}`} className="text-primary-500 hover:text-primary-600 font-bold flex items-center gap-1 uppercase tracking-wider text-[9.5px]">
                  <span>Đọc tiếp</span>
                  <ArrowRight size={12} />
                </Link>
              </div>
